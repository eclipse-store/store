...
| `approximateScoring` | `NONE` | How traversal scores candidates. `FUSED_PQ` writes Product Quantization codes into every node: faster traversal of a large on-disk index, but **makes the graph file larger** and adds transient heap at persist time. `PQ_IN_MEMORY` buys the same traversal with the codes in a `.pqv` sidecar read into heap instead: nothing added to the graph, `pqSubspaces` bytes per ordinal resident. Also the only PQ-based mode an in-memory index can use - `NONE` stays the default there - where it needs `optimizationIntervalMs > 0` and a `minChangesBetweenOptimizations` the index will actually reach |
