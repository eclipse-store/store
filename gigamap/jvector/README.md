...
- **The in-memory switch to `PQ_IN_MEMORY` blocks every write to the GigaMap while it runs**: it is a full rebuild under the map's lock, close to linear in the vector count - about 4 s at 10,000 vectors, 14-19 s at 50,000, several minutes at a million (dimension 64). Searches continue against the pre-switch graph. It happens at most once per index generation, but it is triggered by the *background* optimization, so on a large index call `optimize()` at a quiet moment rather than letting the scheduler pick one.
