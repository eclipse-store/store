...
public class ByteNonPrimitiveIndexTest
{
    @TempDir
    Path tempDir;
    ...
    void nullTests()
    {
        ...
        assertEquals(0, map.query(bytePersonIndex.lessThan(null)).count());
        assertEquals(0, map.query(bytePersonIndex.lessThanEqual(null)).count());
        assertEquals(0, map.query(bytePersonIndex.greaterThan(null)).count());
        assertEquals(0, map.query(bytePersonIndex.greaterThanEqual(null)).count());

        assertEquals(0, map.query(bytePersonIndex.between(null, null)).count());
        assertEquals(0, map.query(bytePersonIndex.between((byte) 30 , null)).count());
        assertEquals(0, map.query(bytePersonIndex.between(null, (byte) 30)).count());
    ...
    void bytePersonTest()
    {
        ...
        try (EmbeddedStorageManager manager = EmbeddedStorage.start(map, tempDir)) {
            map.query(bytePersonIndex.is((byte) 20)).forEach(bytePerson -> assertEquals((byte)20, bytePerson.getAge()));
            map.query(bytePersonIndex.is((byte) 30)).forEach(bytePerson -> assertEquals((byte)30, bytePerson.getAge()));
    ...
    private GigaMap<BytePerson> prepageGigaMap()
    {
        ...
        BytePerson person4 = new BytePerson("David", null);
