...
In case you need to modify the configuration in some way in the program, change something, add and the like, you can have the `EclipseStoreProperties` Spring bean injected into your class. This class contains all the configuration values that are used to create a `StorageManager`.
You can modify these values and then use the `EmbeddedStorageManagerFactory` Bean to create a new `EmbeddedStorageManager` or `EmbeddedStorageFoundationFactory` to create a new  `EmbeddedStorageFoundation`.
...
You can include the Client GUI application to be directly served by your application. For this purpose, you need to activate
the SpringBoot REST Service and include the SpringBoot Console to your application classpath.

Please add the following dependency to your classpath:
...
and configure the path of the console by setting the following properties in your configuration file:
...
The console application starts with your application and will be available at the specified URL.
