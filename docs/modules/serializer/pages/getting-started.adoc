...
The serializer works with almost any Java object — no `java.io.Serializable` interface, no annotations, and no default constructor are required.
Types bound to JVM internals (such as `Thread` or IO streams) are excluded.
See xref:storage:faq/data-model.adoc[Data Model FAQ] for details.
