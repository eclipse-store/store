...
** `set(id, entity)` on an id that `removeById` had emptied restores the size, which stayed one too low - persistently - even though the entity was present and queryable
