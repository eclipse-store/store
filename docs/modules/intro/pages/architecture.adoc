...
Type handlers are generated automatically for almost any Java type.
Classes do not need to implement `java.io.Serializable`, carry annotations, or provide a default constructor; only types tied to JVM internals (e.g. `Thread`, IO streams) and types whose class identity is unstable across runs (lambdas and anonymous inner classes, whose synthetic `$$Lambda$...` / `$1` names are generated by the JVM or compiler) are excluded.
