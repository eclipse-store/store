...
[NOTE]
====
*Lazy loading* (this page) defers reading objects from storage into RAM until they are accessed.
It is unrelated to xref:storage:storing-data/lazy-eager-full.adoc[*lazy storing*], a separate concept that controls how `store()` traverses the object graph when writing.
====
