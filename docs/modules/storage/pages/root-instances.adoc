...
Both `root()` and `setRoot()` use a generic type parameter `<R>`, so the concrete type is inferred automatically from the usage context.
No cast is required:
...
. Starts the storage if it is not yet running.
. If the root is `null`, invokes the supplier to create a new root instance, sets it, and stores it.
. Returns the root object, typed to `R`.
