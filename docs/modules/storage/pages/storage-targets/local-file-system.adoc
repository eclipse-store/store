...
This is the default storage target and the easiest way to start, especially for prototyping and testing purposes.
...
== When to Use Other Storage Targets

The local file system is suitable for:

* Development and testing
* Single-server deployments
* Applications where data does not need to be shared across multiple hosts

For production deployments requiring high availability, shared storage, or cloud deployments, consider the xref:storage-targets/index.adoc[other storage targets] such as S3 or Azure Storage.
