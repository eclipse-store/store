...
|JDK value-based types (String, Number types, Date, File, Locale, Optional, ...)
|image::symbol-check.svg[]
|Optimized handling via custom TypeHandlers. Not to be confused with JEP 401 value classes, see xref:value-classes.adoc[Value Classes].
...
|image::symbol-check.svg[]
|Persisted as entities, and optionally written into their owner rather than referenced. Needs a constructor taking the persistable fields in declaration order, and the preview VM parameters, see xref:value-classes.adoc[Value Classes].

|Value classes as a persistence root
|image::symbol-remove.svg[]
|A value instance has no identity, so persisted state cannot be applied to an explicitly set root. A root that is only loaded resolves normally.
|===
