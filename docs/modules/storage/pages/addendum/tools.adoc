...
== Storages Containing Value Classes

Both tools handle a storage whose data contains xref:value-classes.adoc[Value Classes], and neither needs a JEP 401 JDK or the application's classes to do it.

The Storage Converter copies entities by their header and writes their content through untouched, so a value field written into its owner is carried over without being interpreted.
What both tools do rewrite is the type dictionary, and an inlined field survives that with its `value` keyword, its declaring type and its own layout intact.

[NOTE]
====
The Storage Migrator rewrites the type names in a type dictionary, but not the declaring type qualifier of a member.
This applies to all member kinds, not only inlined ones.
====
