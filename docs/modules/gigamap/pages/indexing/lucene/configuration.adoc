...
When no directory creator is specified, the index data is stored inside the GigaMap's object graph. This means the index is automatically persisted when the GigaMap is stored with {product-name}.
