...
Use Lucene for full-text search and Bitmap for exact filtering. `LuceneIndex.search(...)` returns a `LuceneSearchResult` that is itself a xref:queries/defining.adoc#_sub_queries[`GigaMap.SubQuery`], so the two can be intersected directly.
...
IndexerString<Product> categoryIndexer = new IndexerString.Abstract<>()
{
    @Override
...
// Full-text search intersected with category filter — scores dropped
LuceneSearchResult<Product> textHits = textSearch.search("name:wireless", 100);
List<Product> filtered = products.query(categoryIndexer.is("Electronics"))
    .and(textHits)
    .toList();

// Or from the scored side — keep iterating with relevance scores
ScoredSearchResult<Product> ranked = textHits.and(
    products.query(categoryIndexer.is("Electronics"))
);
----
