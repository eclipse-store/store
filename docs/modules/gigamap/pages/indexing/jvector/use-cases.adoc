...
VectorIndexConfiguration config = VectorIndexConfiguration.builder()
    ...
    .approximateScoring(ApproximateScoring.FUSED_PQ) // Large customer bases benefit from compression
    .pqSubspaces(32)
...
|PQ compression
|Customer databases can be very large, and PQ keeps per-hop I/O low
...
|PQ compression
|Faster traversal over large knowledge bases
...
For datasets with millions of vectors where on-disk search latency is critical. On-disk mode is what lets the index exceed available memory; PQ then reduces the I/O per traversal hop, at the cost of a larger graph file, and NVQ storage brings the file back down well below where it started.
...
VectorIndexConfiguration config = VectorIndexConfiguration.builder()
    .dimension(768)
    .similarityFunction(VectorSimilarityFunction.COSINE)
    .maxDegree(32)                        // Higher recall; also sets the fused block size
    .beamWidth(200)
    .onDisk(true)
    .indexDirectory(Path.of("/data/large-scale"))
    .approximateScoring(ApproximateScoring.FUSED_PQ)
    .pqSubspaces(48)                      // 768 / 48 = 16 dims per subspace
    .vectorStorage(VectorStorage.NVQ)     // ~1.9x smaller than INLINE here; scores stay exact
    .parallelOnDiskWrite(true)            // Absorbs almost all of the NVQ encode cost
    .persistenceIntervalMs(60_000)
...
|`FUSED_PQ` scoring
|Speeds up traversal on a large on-disk index (at the cost of a larger graph file)

|`NVQ` storage
|Shrinks the vector block about 3.8x, but the fused codes above are unaffected, so the node as a whole goes from 4,744 to 2,472 bytes -- about 1.9x. Drop `FUSED_PQ` as well and it is 936 bytes, about 3.4x smaller than a plain `INLINE` node. Here the fused codes drive traversal, so quantizing the block does not change which candidates are found; it costs footprint only, and the rerank reads the GigaMap rather than the graph. Scores stay exact

|Parallel on-disk write
|The NVQ encode runs per node on the writer's threads; without this the persist takes about 3x as long rather than about 1.2x
...
|PQ compression
|Keeps queries fast over large image collections
...
[options="header",cols="2,1,1,1,1,1,1,1"]
|===
|Use Case |Similarity |maxDegree |beamWidth |On-Disk |Scoring |Storage |Background
