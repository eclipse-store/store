...
|`NVQ` storage
|Shrinks the vector block about 3.8x, but the fused codes above are unaffected, so the node as a whole goes from 4,744 to 2,472 bytes -- about 1.9x. Drop `FUSED_PQ` as well and it is 936 bytes, about 3.4x smaller than a plain `INLINE` node. Scores stay exact either way; what quantization affects is which candidates traversal finds, so raise the search beam if recall matters more than the footprint
