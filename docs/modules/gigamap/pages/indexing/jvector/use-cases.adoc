...
VectorIndexConfiguration config = VectorIndexConfiguration.builder()
    ...
    .vectorStorage(VectorStorage.NVQ)     // ~3.4x smaller graph; reranking becomes approximate
...
|`NVQ` storage
|About 3.4x fewer bytes per node, so more of the graph stays in page cache. Reranking is then approximate - drop it if the last fraction of a percent of recall matters more than the footprint
