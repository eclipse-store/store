...
VectorIndexConfiguration config = VectorIndexConfiguration.builder()
    ...
    .vectorStorage(VectorStorage.NVQ)     // ~3.4x smaller graph; scores stay exact
...
|`NVQ` storage
|About 3.4x fewer bytes per node, so more of the graph stays in page cache. Scores stay exact; what quantization affects is which candidates traversal finds, so raise the search beam or drop it if the last fraction of a percent of recall matters more than the footprint
