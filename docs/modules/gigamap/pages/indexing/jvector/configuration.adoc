...
|`enablePqCompression`
|`false`
|Enable Product Quantization. Speeds up traversal of a large on-disk index, at the cost of a larger graph file.

|`pqSubspaces`
|`0`
|Number of PQ subspaces (0 = auto: dimension/4). Costs `pqSubspaces * maxDegree` bytes per node, so prefer a smaller value than classic PQ guidance suggests.
