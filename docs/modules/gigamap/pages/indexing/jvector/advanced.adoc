...
NOTE: The on-disk graph file format is at version 4. Indices written by earlier versions are detected on load and rebuilt automatically from the GigaMap-stored source vectors -- no data loss, but expect a cold-start cost on the first restart after upgrade. The rebuilt graph replaces the stale files only once something persists it: with the default `persistOnShutdown(true)`, or with background persistence, that happens on its own and the cost is paid once. If both are disabled and `persistToDisk()` is never called, the old files remain and the rebuild repeats on every restart.
