...
== Defining by Annotation

A spatial index can also be declared without a hand-written `SpatialIndexer` by placing `@SpatialIndex` on the entity type and naming the latitude and longitude properties (fields or no-argument getters):
...
GigaMap<Location> gigaMap = GigaMap.New();
IndexerGenerator.AnnotationBased(Location.class).generateIndices(gigaMap.index().bitmap());

// the generated index is named "spatial" by default (override via @SpatialIndex(name = ...))
SpatialIndexer<Location> spatial = gigaMap.index().bitmap().getIndexer(SpatialIndexer.class, "spatial");
gigaMap.query(spatial.near(40.7128, -74.0060, 100.0));
----

See xref:indexing/annotations.adoc[Annotation-based Indexing] for the overall model.
...
The spatial indexer works with {product-name} persistence out of the box.
...
{
    GigaMap<Location> loaded = storage.root();
