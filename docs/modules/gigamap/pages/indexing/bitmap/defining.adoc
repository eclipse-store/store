...
Any other type falls back to a generic, equality-based indexer (value `equals`/`hashCode`). For such custom key types, make sure `equals` and `hashCode` are implemented.
...
`@Index(binary = true)` is the backward-compatible shortcut for `kind = IndexKind.BINARY`. Requesting `BINARY` or `BIT_SLICED` for an unsupported type fails fast at generation time.
