...
For high cardinality with range query support, the byte-decomposed indexers combine efficient storage with range operations (`lessThan`, `greaterThan`, `between`, etc.):
...
For geographic coordinate indexing with spatial queries (bounding box, proximity, range), the xref:indexing/spatial/index.adoc[spatial indexer] is available:
...
|SpatialIndexer
|Latitude/longitude coordinate pairs (Double)
