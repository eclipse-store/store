...
IMPORTANT: An identity index is **not** a unique index. It only defines which fields are used to identify entities — it does not enforce uniqueness. To enforce uniqueness, add a unique constraint as described in xref:constraints.adoc#_unique_constraints[Constraints].
...
To make an identity index also enforce uniqueness, see xref:constraints.adoc#_unique_constraints[Unique Constraints].
