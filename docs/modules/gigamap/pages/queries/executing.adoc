...
To process each result together with its entity id, use `iterateIndexed`. The consumer receives the id and the entity for every match.

[source, java]
----
GigaQuery<Person> query = gigaMap.query(...);

query.iterateIndexed((id, person) -> {
    log.info("hit {} -> {}", id, person);
});
----

At the iterator level, the same is available via `GigaIterator.nextIndexed(EntryConsumer)`.
...
try(GigaIterator<Person> iterator = query.iterator())
{
    while(iterator.hasNext())
    {
        iterator.nextIndexed((id, person) -> ...);
    }
}
----
